-- =============================================================================
-- SubterraDB control-plane schema
-- =============================================================================
-- Loaded automatically by the postgres image on first boot via the
-- /docker-entrypoint-initdb.d hook. Runs AFTER 01-create-databases.sql so
-- the subterradb_system database already exists.
--
-- Idempotent: every CREATE uses IF NOT EXISTS where possible. Re-running on
-- an existing database is a no-op.
--
-- For future schema changes, prefer adding new files (03-..., 04-...) so the
