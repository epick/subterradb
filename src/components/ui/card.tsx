...
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        'rounded-2xl border border-border/60 bg-card/80 text-card-foreground shadow-2xl shadow-black/40 backdrop-blur-xl',
        className,
      )}
      {...props}
    />
...
const CardFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('flex items-center justify-center gap-2 p-8 pt-0 text-sm', className)}
      {...props}
    />
  ),
);
